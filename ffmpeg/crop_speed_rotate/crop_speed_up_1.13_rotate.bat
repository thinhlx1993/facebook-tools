...
ffmpeg -i %1 -vf "[0:v]rotate=-%rotate%*PI/180,crop=in_w*%crop%:in_h*%crop%,scale=%width%:%height%,setpts=1/%speed%*PTS" -af "[0:a]atempo=%speed%" -preset ultrafast -crf 24 -shortest done/%1 -y