...
if __name__ == '__main__':
    ...
    while number_invited < 150:
        if check_exist("like.PNG", region=(800, 0, 70, 800)):
            click_to("like.PNG", region=(800, 200, 70, 800))

            waiting = 0
            while waiting <= 3:
                number_invited += click_many("invite.PNG", region=(800, 200, 600, 800))
                pyautogui.moveTo(x=1091, y=573, duration=0.5)
                pyautogui.scroll(-400)
                pyautogui.scroll(-400)
                time.sleep(1)
                if not check_exist("invite.PNG", region=(800, 200, 600, 800)):
                    waiting += 1
            if check_exist("close_invite.PNG", region=(800, 200, 600, 800)):
                click_to("close_invite.PNG", region=(800, 200, 600, 800))
