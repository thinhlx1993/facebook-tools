...
if __name__ == '__main__':
    number_invited = 0
    default_region = (800, 0, 70, 800)
    while number_invited < 150:
        if waiting_for("like.PNG", region=default_region, waiting_time=10):
            click_to("like.PNG", region=default_region)

            waiting = 0
            while waiting <= 3:
                number_invited += click_many("invite.PNG", region=(800, 0, 600, 800))
                pyautogui.moveTo(x=1091, y=573, duration=0.5)
                pyautogui.scroll(-400)
                pyautogui.scroll(-400)
                time.sleep(1)
                if not check_exist("invite.PNG", region=default_region):
                    waiting += 1
            if check_exist("close_invite.PNG", region=default_region):
                click_to("close_invite.PNG", region=default_region)
